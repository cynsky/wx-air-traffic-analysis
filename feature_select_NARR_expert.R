		...
		for (day_index in 1:length(days)) {
			...
			for (time_index in 1:length(timestamps)) {
				...
				if (file.exists(fname)) {
					...
					atl_vec_1 = c(nyc_vec_1,rep(NA,3))
					usa_vec_1 = c(nyc_vec_1,rep(NA,6))
					nyc_route_precip = c(nyc_vec_3,rep(NA,length(NY_grid_route)))
					atl_route_precip = c(atl_vec_3,rep(NA,length(ATL_grid_route)))
					usa_route_precip = c(usa_vec_3,rep(NA,length(USA_grid_route)))
					nyc_route_cape = c(nyc_vec_3,rep(NA,length(NY_grid_route)))
					atl_route_cape = c(atl_vec_3,rep(NA,length(ATL_grid_route)))
					usa_route_cape = c(usa_vec_3,rep(NA,length(USA_grid_route)))
					nyc_precip_vec = c(nyc_vec_3,rep(NA,length(NYways$lat)))
					atl_precip_vec = c(atl_vec_3,rep(NA,length(ATLways$lat)))
					usa_precip_vec = c(usa_vec_3,rep(NA,length(waypoints$lat)))
					nyc_cape_vec = c(nyc_vec_3,rep(NA,length(NYways$lat)))
					atl_cape_vec = c(atl_vec_3,rep(NA,length(ATLways$lat)))
					usa_cape_vec = c(usa_vec_3,rep(NA,length(waypoints$lat)))
				}
			}
