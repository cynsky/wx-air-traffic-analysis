...
# Remove the date vector from any data frame that has it
# raw_pca1 = raw_pca1[,2:15]
# raw_pca2 = raw_pca2[,2:11]

# Combine the relevant features into one data frame
raw_features = data.frame(raw_exp2,raw_exp3)
...
# exp2 = scale(raw_exp2)
# exp3 = scale(raw_exp3)
features = scale(raw_features)

# Apply k medoid clustering, and study the average silhouette width
# library(cluster)
# asw = 1
# for (k in 2:20) { asw[k]=pam(exp3,k)$silinfo$avg.width }

# Apply k means clustering, and select k using the elbow method based on the wss
...
# Apply k means clustering, and select k by studying the BIC
...
BIC = (nrow(features)-1)*sum(apply(features,2,var))
for (i1 in 2:50) {
  new_model = kmeans(features,centers=i1)
  BIC[i1] = kmeansBIC(new_model)
}
ideal_k = which.min(BIC)
pdf("/Users/kkuhn/Desktop/SelectK.pdf",width=7,height=4)
par(mai=c(0.9,0.9,0.1,0.1))
plot(BIC,xlab="k",ylab="BIC",main="",pch=20)
points(ideal_k,BIC[ideal_k],pch=8,col="red")
dev.off()

features_kmeans = kmeans(features,centers=ideal_k)
kmeans_results = data.frame(date=d_list,cluster=features_kmeans$cluster,raw_features)
...
setwd(output_dir)
write.csv(kmeans_results,file="NY_RUC_kmeans.csv",row.names=F)
